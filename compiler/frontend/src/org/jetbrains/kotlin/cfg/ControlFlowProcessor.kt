        ...
        */
        ...
        private fun generateQualifier(expression: KtExpression, qualifier: Qualifier): Boolean {
            ...
            if (qualifierDescriptor is ClassDescriptor) {
                when (qualifierDescriptor.kind) {
                    ClassKind.ENUM_ENTRY, ClassKind.OBJECT -> {
                        mark(expression)
                        builder.read(expression, AccessTarget.Declaration(FakeCallableDescriptorForObject(qualifierDescriptor)), emptyMap())
                        return true
                    }
                    else -> {}
                }
