    ...
    private fun computeDeclaredVariablesForPseudocode(pseudocode: Pseudocode): Set<VariableDescriptor> {
        ...
        for (instruction in pseudocode.instructions) {
            if (instruction is VariableDeclarationInstruction) {
                ...
                when (descriptor) {
                    is VariableDescriptor -> declaredVariables.add(descriptor)
                    is ClassDescriptor -> when (descriptor.kind) {
                        ClassKind.ENUM_ENTRY, ClassKind.OBJECT -> declaredVariables.add(FakeCallableDescriptorForObject(descriptor))
                        else -> {}
                    }
                }
            }
