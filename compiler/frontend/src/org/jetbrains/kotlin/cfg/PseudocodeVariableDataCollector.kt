            ...
            if (instruction is VariableDeclarationInstruction) {
                ...
                when (descriptor) {
                    is VariableDescriptor ->
                            blockScopeVariableInfo.registerVariableDeclaredInScope(descriptor, instruction.blockScope)
                    is ClassDescriptor ->
                            if (descriptor.kind == ClassKind.OBJECT || descriptor.kind == ClassKind.ENUM_ENTRY) {
                                val variableDescriptorForObject = FakeCallableDescriptorForObject(descriptor)
                                blockScopeVariableInfo.registerVariableDeclaredInScope(variableDescriptorForObject, instruction.blockScope)
                            }
                    else -> throw AssertionError(
                                "Variable or class descriptor should correspond to the instruction for ${instruction.element.text}.\n" +
